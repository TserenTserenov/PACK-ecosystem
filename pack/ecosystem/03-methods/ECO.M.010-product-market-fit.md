...
| Force | Tension |
|-------|---------|
| Частота замера ↔ качество сигнала | Редкие замеры Sean Ellis Test (§3: ежеквартально) снижают шум, но задерживают обнаружение потери PMF между циклами |
| Единая метрика как proof ↔ конвергенция сигналов | Стейкхолдерам проще один KPI, но Multi-Signal Framework (§2) требует 3+ сигналов одновременно — единственная хорошая метрика ведёт к FM.PMF.002 |
| Скорость получения сигнала ↔ информативность опроса | Желание узнать PMF быстрее конкурирует с правилом «после 7-14 дней, не в первый день» (§3) — ранний опрос неинформативен (FM.PMF.003) |
...
|------|--------------------------|
| Одна убедительная метрика вместо конвергенции | Внимание съезжает на единственный сильный сигнал (высокий NPS или высокий Sean Ellis) и недооценивается требование конвергенции 3+ сигналов (§2) — прямой путь к FM.PMF.002 |
| Power users вместо репрезентативной выборки | При Sean Ellis Test (§3) внимание смещается к охотно отвечающим power users, тогда как менее вовлечённая часть аудитории выпадает из опроса (FM.PMF.004) |
| Средние показатели вместо формы кривой | Внимание переоценивается усреднённый retention (§4) — «средний D30 20%» выглядит нормально, хотя если кривая не выходит на плато, PMF на самом деле нет |
