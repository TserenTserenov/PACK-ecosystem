...
| Force | Tension |
|-------|---------|
| Clarity ↔ Accuracy | «Clarity 100% > Accuracy 100%» (§7) — упрощение pitch до понятного за 60 сек неизбежно теряет нюансы, которые важны для due diligence позже |
| Скорость привлечения ↔ качество условий (dilution) | Быстрое закрытие раунда (SAFE, недели) снижает transaction cost, но обычно даёт менее выгодные условия, чем медленный priced round с полным due diligence |
| Оптимистичный нарратив ↔ достоверность прогноза | Инвесторская психология вознаграждает уверенный traction-нарратив (§9), но bottom-up модель (§8) требует признавать риски и sensitivity — избыточная уверенность превращается в FM.FR.011 (no sensitivity analysis) |
...
|------|--------------------------|
| Vanity metrics вместо traction | Внимание непроизвольно съезжает на легко предъявляемые числа (downloads, регистрации) вместо труднее добываемых, но значимых (revenue, retention) — см. D.FR.001 |
| Top-down projection как оптимизм | «$100B TAM × 1%» кажется убедительным основателю именно потому, что не требует признания узких мест воронки; bottom-up модель (cohort × conversion × ARPU) вскрывает слабые места, которые top-down прячет |
